...
#define T 3		// Given time, unit: sec
...
double reference[7] = {0, -M_PI_4, 0, -3 * M_PI_4, 0, M_PI_2 , M_PI_4};
...
void *control_thread(void *arg){
    ...
    // robot.control
		...
		robot.control([&x, &x_old, &x_control, &x_control_last, &timer, &h](const franka::RobotState& robot_state,
			franka::Duration period) -> franka::JointVelocities
		{
			...
			for (i=0;i<7;i++){
				if (timer < T) x_control_last[i] = x_control[i];
				else x_control_last[i].v = 0.0;
				fprintf(ctrl_fp," %f",x_control_last[i].v);
			}			
			fprintf(ctrl_fp,"%c\n",59);
			franka::JointVelocities output = {{x_control_last[0].v,x_control_last[1].v,x_control_last[2].v,x_control_last[3].v, x_control_last[4].v, x_control_last[5].v, x_control_last[6].v}};
...
void *MPC_thread(void *arg){
	...
	while (!StopFlag){
		...
		for (i=0;i<7;i++){
			...
			if (i==3) {
				fprintf(mpc_fp, " %f %f %f %f %f %f %f %f %f %f", vars.x_1[1], vars.x_2[1], vars.x_3[1],vars.x_4[1], vars.x_5[1], vars.x_6[1],vars.x_7[1], vars.x_8[1], vars.x_9[1],vars.x_10[1]);
				fprintf(mpc_fp, " %f %f %f %f %f %f %f %f %f %f", vars.x_11[1], vars.x_12[1], vars.x_13[1],vars.x_14[1], vars.x_15[1], vars.x_16[1],vars.x_17[1], vars.x_18[1], vars.x_19[1],vars.x_20[1]);				
				fprintf(mpc_fp, " %f %f %f %f %f", vars.x_21[1], vars.x_22[1], vars.x_23[1],vars.x_24[1], vars.x_25[1]);
			} 
...
int main(int argc, char** argv) 
{
	...
	printf("------------DONE-------------\n");	
	for (int i=0; i<7; i++){
		if (fabs(q_final[i]-reference[i])<1e-6) err=0; 
		else err = (q_final[i]-reference[i])/reference[i]*100;
